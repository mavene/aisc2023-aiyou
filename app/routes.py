...
from app import application, engine, sentic_bert, gen_cloud
...
def search():
    if request.method == "POST":
        ...
        sentiment_analysis = sentic_bert.inference(result)
        get_started = "" 
    else:
        result = ""
        sentiment_analysis = ""
...
def details():
    if 'new-search' in request.form:
        ...
        parsed_detail = [int(elem) for elem in detail.split(",")]
        redirect_entity = Entity.query.filter(Entity.id.like(parsed_detail[0])).first()
        redirect_sentiment = parsed_detail[1:]
        all_reviews = engine.find_reviews(redirect_entity)
